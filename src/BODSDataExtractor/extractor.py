# LLD see comments on other files r.e. header.function
# also please ensure that functions are in relevance order (following __init__) 
# helper/reusable methods should either be at the end or in a Helper object (preferred,
# if you still want to use the object-oriented route)
# 
# 
import pandas as pd
import json
...
# LLD don't need method imports when package imported; from datetime import date
import datetime
from collections import Counter
import importlib.resources

# LLD please provide comment explaining at high level what this class does
# Need to ensure that we stick to the 'cohesion' principle - that an object does
# one thing, well - rather than doing multiple things (worth a Google!)
class TimetableExtractor:

    error_list = []

    # LLD per my comments in the example.py file, the main functionality should not take place
    # within __init__. Use this only for setting attributes where really necessary.
    # A usual practice would be an almost empty init, perhaps with the api_key included.
    # All the other attributes could then be called via a get_timetables() method.
    # The object could then easily be used with different parameters to do more
    # timetable queries. It would still be possible to store the attributes of the
    # method call in the object for each API call.
    def __init__(
        self,
        api_key,
        limit=10000,
        ...
        # LLD do initiate the bods_client as an attribute in this method as it is reusable

        # LLD - this should definitely be removed into a separate method (ie taken out the instantiator). 
        # Makes error tracking much easier and follows standard practice of cohesion.
        self.pull_timetable_data()

        # you should only do what is necessary to fulfill user need. If this is only used for analysis or for data quality, should move
        # and allow user to call themselves if needed (or if part of a data quality function, use it in there)
        self.otc_db = otc_db_download.fetch_otc_db()

        
        # LLD - the test here should either be if service_line_level or use is instead of ==
        # I have included both examples below
        if service_line_level:
            self.analytical_timetable_data()
            self.analytical_timetable_data_analysis()
        
        # LLD - this else is not required - the above code block will only execute if the condition is true
        else:
            pass

        if stop_level is True:
            self.analytical_timetable_data()
            self.analytical_timetable_data_analysis()
            self.generate_timetable()

            # LLD I need to continue reviewing to determine what the above methods do, but we need to 
            # separate any timetable extracting code from any 'analytical' (DQ?) code.


        
# LLD remove test code if no longer required
        # self.service_line_extract = service_line_extract


# LLD rename method to more accurately reflect what it does. Think about reuse and cohesion... 
# could this method be used by any object to create a df from JSON? 
# If so, check what already exists, and if still needed, move this to a Helper object class.
...
# LLD again, this could be re-written as a reusable helper function. See comment above.
    def determine_file_type(self, url):
        ...
        try:
            ...
            return extension
        # LLD no bare exceptions: catch the exceptions you know can happen
        except:
            return "error in filename"


# LLD please provide clear description of the intended 
# functionality of this method - in a docstring
# as you have for other methods is good approach. Ideally docstrings should also 
# advise on input and output variables.
# Once the description has been written, please get me to re-review
...
# LLD this should be the second method after init as it is the most important and the overall
# 'orchestrator' method.
# in example.py, after the object is created, this should be the first call. However,
# as epxlained in my comment on the initiator method, this method should have
# many more arguments (basically all the ones initiator has except api)
...
# LLD this should actually be in __init__ because it can be reused
        # instantiate a BODSClient object
        bods = BODSClient(api_key=self.api_key)

# LLD these would be replaced as they'll be arguments of the method. You can
# still save to the object attributes if you want to, but you don't have to.
        params = timetables.TimetableParams(
            limit=self.limit, nocs=self.nocs, status=self.status, search=self.search
        )
...
# LLD consider whether you want to work with self.metadata or a copy of self.metadata
        self.metadata = TimetableExtractor.create_zip_level_timetable_df(self, data)
        print(
            f"metadata downloaded for {len(self.metadata['url'])} records, converting to dataframe...\n"
        )

# LLD what does this actually mean to the user (the print statement?)
        print("appending filetypes...\n")
        self.metadata["filetype"] = [
            TimetableExtractor.determine_file_type(self.metadata, x)
            for x in self.metadata["url"]
        ]
# LLD use the if template mentioned above (remove == True or change to is True)
# is there no way to filter this in the API pull?
        # limit to just bods compliant files if requested
        if self.bods_compliant: #lld not needed == True:
            self.metadata = self.metadata[self.metadata["bods_compliance"]] # LLD not needed == True]
        #lld not needed:
        else:
            pass
# LLD is there no way to filter this within the api call?
        ...
        if self.atco_code is not None:
            ...
            exploded_metadata = TimetableExtractor.xplode(
                exploded_metadata, ["admin_areas"]
            )
            # use exploded out atco codes to filter for only the requested ones
            exploded_metadata = exploded_metadata[
                exploded_metadata["admin_areas"].isin(self.atco_code)
            ]

            # filter the output metadata dataframe by the atco codes
            self.metadata = self.metadata[
                self.metadata["id"].isin(exploded_metadata["id"])
            ]
        # LLD else: pass is never needed :)
        else:
            pass

        return self.metadata

# LLD for readability, please ensure that methods are arranged from highest level to lowest level of detail, ideally
# in the order that they are called from the highest level of detail.

# LLD START MONDAY (SECTION 1)

# LLD this function doesn't appear to actually be used anywhere, is that correct? If not, 
# rework required as outlined below
...
# LLD why is the user of this method required to pass an empty list? This should be instantiated within the method
        Arguments: URL and a list in which to pass urls which could not be treated as xmls
        """

        try:
            # LLD please use requests.head() instead of get() to get the metadata -
            # quicker because don't download the content at all, 
            # just the metadata itself. Rest of method will then change
            # See examples here https://www.codecademy.com/resources/docs/python/requests-module/head
...
# LLD please ensure named exception type caught, not just any exception as this hides errors
        except:
            #LLD this should handle a specific error, and we are better using an if-then if we 
            # know what we are checking for. Check using regex for ending of .xml.
                ...
                if extension == "xml":
                    ...
                    with thezip.open(zipinfo) as thefile:
                        ...
                        try:
                            ...
                            trading_name = xmlDataExtractor.extract_trading_name(
                                xml_data
                            )
                            xml_output.append(trading_name)

                            licence_number = xmlDataExtractor.extract_licence_number(
                                xml_data
                            )
                            xml_output.append(licence_number)

                            operator_short_name = (
                                xmlDataExtractor.extract_operator_short_name(xml_data)
                            )
                            xml_output.append(operator_short_name)

                            operator_code = xmlDataExtractor.extract_operator_code(
                                xml_data
                            )
                            xml_output.append(operator_code)

                            service_code = xmlDataExtractor.extract_service_code(
                                xml_data
                            )
                            ...
                            service_origin = xmlDataExtractor.extract_service_origin(
                                xml_data
                            )
                            xml_output.append(service_origin)

                            service_destination = (
                                xmlDataExtractor.extract_service_destination(xml_data)
                            )
                            xml_output.append(service_destination)

                            operating_period_start_date = (
                                xmlDataExtractor.extract_operating_period_start_date(
                                    xml_data
                                )
                            )
                            ...
                            operating_period_end_date = (
                                xmlDataExtractor.extract_operating_period_end_date(
                                    xml_data
                                )
                            )
                            xml_output.append(operating_period_end_date)

                            schema_version = xmlDataExtractor.extract_schema_version(
                                xml_data
                            )
                            xml_output.append(schema_version)

                            revision_number = xmlDataExtractor.extract_revision_number(
                                xml_data
                            )
                            ...
                            if self.stop_level == True:
                                ...
                                journey_pattern_json = xml_json["TransXChange"][
                                    "JourneyPatternSections"
                                ]["JourneyPatternSection"]
                                xml_output.append(journey_pattern_json)

                                vehicle_journey_json = xml_json["TransXChange"][
                                    "VehicleJourneys"
                                ]["VehicleJourney"]
                                xml_output.append(vehicle_journey_json)

                                services_json = xml_json["TransXChange"]["Services"][
                                    "Service"
                                ]
    ...
    def download_extract_xml(self, url):
        ...
        with open(r"temp.xml", "r", encoding="utf-8") as xml:
            ...
            operating_period_start_date = (
                xmlDataExtractor.extract_operating_period_start_date(xml_data)
            )
            xml_output.append(operating_period_start_date)

            operating_period_end_date = (
                xmlDataExtractor.extract_operating_period_end_date(xml_data)
            )
            xml_output.append(operating_period_end_date)
            ...
            if self.stop_level == True:
                ...
                vehicle_journey_json = xml_json["TransXChange"]["VehicleJourneys"][
                    "VehicleJourney"
                ]
                xml_output.append(vehicle_journey_json)
    ...
    def analytical_timetable_data(self):
        ...
        xml_table = [
            TimetableExtractor.download_extract_xml(self, x) for x in xml_table["URL"]
        ]
        try:
        ...
        zip_table = [
            TimetableExtractor.download_extract_zip(self, x) for x in zip_table["URL"]
        ]
        try:
        ...
        self.service_line_extract_with_stop_level_json = master_table.merge(
            zip_xml_table, how="outer", on="URL"
        )
        ...
        if self.atco_code is not None:
            self.service_line_extract_with_stop_level_json = (
                self.service_line_extract_with_stop_level_json[
                    self.service_line_extract_with_stop_level_json["la_code"].isin(
                        self.atco_code
                    )
                ]
            )
    ...
    def unwrap_journey_pattern_json(self, json):
        ...
        for js in range(0, len(json)):
            ...
            for jptl in range(0, len(json[js]["JourneyPatternTimingLink"])):
                ...
                stop_from.append(
                    json[js]["JourneyPatternTimingLink"][jptl]["From"]["StopPointRef"]
                )
                stop_to.append(
                    json[js]["JourneyPatternTimingLink"][jptl]["To"]["StopPointRef"]
                )
                # sequence no and timing status not mandated by schema so must use try except
                try:
                    sequence_number.append(
                        json[js]["JourneyPatternTimingLink"][jptl]["From"][
                            "@SequenceNumber"
                        ]
                    )
                except:
    ...
    def unwrap_vehicle_journey_json_for_runtime(self, json):
        ...
        for v in range(0, len(json)):
            ...
            for vjtl in range(0, len(json[v]["VehicleJourneyTimingLink"])):
                ...
                JourneyPatternRef.append(json[v]["JourneyPatternRef"])
                jptl_id.append(
                    json[v]["VehicleJourneyTimingLink"][vjtl][
    ...
    def unwrap_service_json(self, json):
        ...
        if type(json["StandardService"]["JourneyPattern"]) is list:
            ...
            for jp in range(0, len(json["StandardService"]["JourneyPattern"])):
                ...
                jpsf_id.append(
                    json["StandardService"]["JourneyPattern"][jp][
            ...
            jpsf_id.append(
                json["StandardService"]["JourneyPattern"]["JourneyPatternSectionRefs"]
            )
    ...
    def produce_stop_level_df_journey(self):
        ...
        full_data_extract_no_la_code = (
            self.service_line_extract_with_stop_level_json.drop("la_code", axis=1)
        )
        ...
        for i in range(0, len(full_data_extract_no_la_code)):
            try:
                stop_level_list_journey_pre = (
                    TimetableExtractor.unwrap_journey_pattern_json(
                        self, full_data_extract_no_la_code["journey_pattern_json"][i]
                    )
                )
            except:
                print(
                    full_data_extract_no_la_code["DatasetID"][i],
                    full_data_extract_no_la_code["FileName"][i],
                    "journey extraction error",
                )
                pass

            # add relevant meta data from that file to the unwrapped journey pattern details
            stop_level_list_journey_pre["DatasetID"] = full_data_extract_no_la_code[
                "DatasetID"
            ][i]
            stop_level_list_journey_pre["FileName"] = full_data_extract_no_la_code[
                "FileName"
            ][i]
            stop_level_list_journey_pre["ServiceCode"] = full_data_extract_no_la_code[
                "ServiceCode"
            ][i]
            stop_level_list_journey_pre["LineName"] = full_data_extract_no_la_code[
                "LineName"
            ][i]
            stop_level_list_journey_pre[
                "RevisionNumber"
            ] = full_data_extract_no_la_code["RevisionNumber"][i]
        ...
        stop_level_df_journey = TimetableExtractor.xplode(
            stop_level_df_journey,
            [
    ...
    def produce_stop_level_df_vehicle(self):
        ...
        for i in range(0, len(full_data_extract_no_la_code)):
            try:
                stop_level_list_vehicle_pre = (
                    TimetableExtractor.unwrap_vehicle_journey_json(
                        self, full_data_extract_no_la_code["vehicle_journey_json"][i]
                    )
                )
            except:
                print(
            ...
            stop_level_list_vehicle_pre["DatasetID"] = full_data_extract_no_la_code[
                "DatasetID"
            ][i]
            stop_level_list_vehicle_pre["FileName"] = full_data_extract_no_la_code[
                "FileName"
            ][i]
            stop_level_list_vehicle_pre["ServiceCode"] = full_data_extract_no_la_code[
                "ServiceCode"
            ][i]
            stop_level_list_vehicle_pre["LineName"] = full_data_extract_no_la_code[
                "LineName"
            ][i]
            stop_level_list_vehicle_pre[
                "RevisionNumber"
        ...
        # explode out lists in dataframe
        stop_level_df_vehicle = TimetableExtractor.xplode(
            stop_level_df_vehicle,
            ["VehicleJourneyCode", "JourneyPatternRef", "DepartureTime", "LineRef"],
        )
    ...
    def produce_stop_level_df_vehicle_for_runtime(self):
        ...
        for i in range(0, len(full_data_extract_no_la_code)):
            try:
                stop_level_list_vehicle_pre = (
                    TimetableExtractor.unwrap_vehicle_journey_json_for_runtime(
                        self, full_data_extract_no_la_code["vehicle_journey_json"][i]
                    )
                )
            except:
                ...
                stop_level_list_vehicle_pre = {
                    "JourneyPatternRef": "N/A",
                    "LineRef": "N/A",
                    "journey_pattern_timing_link": "N/A",
                    "runtime": "N/A",
                }
    ...
    def produce_stop_level_df_service(self):
        ...
        for i in range(0, len(full_data_extract_no_la_code)):
            ...
            try:
                stop_level_list_service_pre = TimetableExtractor.unwrap_service_json(
                    self, full_data_extract_no_la_code["services_json"][i]
                )
            except:
                print(
                    full_data_extract_no_la_code["DatasetID"][i],
            ...
            stop_level_list_service_pre["DatasetID"] = full_data_extract_no_la_code[
                "DatasetID"
            ][i]
            stop_level_list_service_pre["FileName"] = full_data_extract_no_la_code[
                "FileName"
            ][i]
            stop_level_list_service_pre["ServiceCode"] = full_data_extract_no_la_code[
                "ServiceCode"
            ][i]
            stop_level_list_service_pre["LineName"] = full_data_extract_no_la_code[
                "LineName"
            ][i]
            stop_level_list_service_pre[
                "RevisionNumber"
            ] = full_data_extract_no_la_code["RevisionNumber"][i]
        ...
        # explode out lists in dataframe
        stop_level_df_service = TimetableExtractor.xplode(
            stop_level_df_service, ["JourneyPattern_id", "JourneyPatternSectionRef"]
        )
        stop_level_df_service = stop_level_df_service.explode(
    ...
    def join_stop_level_data(self):
        ...
        vehicle_with_runtime_stop_level = (
            TimetableExtractor.produce_stop_level_df_vehicle_for_runtime(self)
        )
        ...
        stop_level_joined = vehicle_stop_level.merge(
            service_stop_level[
                [
                    "JourneyPattern_id",
                    "JourneyPatternSectionRef",
        ...
        stop_level_joined = stop_level_joined[
            ~stop_level_joined["sequence_number"].isnull()
        ]

        # convert data type of seq number to int
        stop_level_joined["sequence_number"] = stop_level_joined[
            "sequence_number"
        ].astype("int")
    ...
    def clean_stop_level_data(self):
        ...
        stop_level_joined_clean["runtime_x"] = (
            stop_level_joined_clean["runtime_x"].astype("str").str.extract(r"(\d+)")
        )
        stop_level_joined_clean["runtime_y"] = (
            stop_level_joined_clean["runtime_y"].astype("str").str.extract(r"(\d+)")
        )

        # coalesce these cols - PTI logic dictates that if there is no runtime in the vehicle frame, then use the journey frame
        stop_level_joined_clean["runtime"] = stop_level_joined_clean[
            "runtime_y"
        ].combine_first(stop_level_joined_clean["runtime_x"])
        ...
        stop_level_joined_clean.loc[
            stop_level_joined_clean["sequence_number"] == 1, "runtime"
    ...
    def pivot_clean_stop_level_data(self):
        ...
        stop_level_clean_pivoted["ServiceCode_LineName_RevisionNumber"] = (
            stop_level_clean_pivoted["ServiceCode"]
            + "_"
            + stop_level_clean_pivoted["LineName"]
            + "_"
        ...
        stop_level_clean_pivoted = stop_level_clean_pivoted[
            [
                ...
                "LineName",
                "RevisionNumber",
                "ServiceCode_LineName_RevisionNumber",
                "VehicleJourneyCode",
                "sequence_number",
                "stop_from",
                "runtime",
        ...
        stop_level_clean_pivoted = (
            stop_level_clean_pivoted.groupby(
                [
                    "DatasetID",
                    "ServiceCode_LineName_RevisionNumber",
                    "ServiceCode",
    ...
    def generate_timetable(self):
        ...
        for dataset in set(stop_level_clean_pivoted.reset_index()["DatasetID"]):
            timetable_df_pre_pre = stop_level_clean_pivoted.loc[[dataset]].dropna(
                axis=1, how="all"
            )

            # split for each service code, so that final timetable output is specific to each service code
            for service in set(
                timetable_df_pre_pre.reset_index()[
                    "ServiceCode_LineName_RevisionNumber"
                ]
            ):
                timetable_df_pre = timetable_df_pre_pre.loc[
                    (slice(None), [service]), :
                ...
                for c in timetable_df_pre.columns:
                    ...
                    timetable_df = timetable_df.merge(
                        timetable_df_temp, how="left", left_index=True, right_index=True
                    )
                ...
                timetable_df = timetable_df.applymap(
                    lambda x: x.strftime("%H:%M") if x != "null" else np.nan
                ).reset_index()

                # merge to get lon and lat of stop_from
                # timetable_df = timetable_df.merge(naptan, how='left',left_on='stop_from',right_on='ATCOCode')
                timetable_df = naptan.merge(
                    timetable_df, how="right", right_on="stop_from", left_on="ATCOCode"
                )
            ...
            self.timetable_dict = {
                f"{i}_{j}": k
                for i, j, k in zip(list_of_datasets, list_of_service_codes, list_of_dfs)
            }

        print("Timetable generated!")
        return self.timetable_dict

# LLD END MONDAY (SECTION 1)

# LLD move these to a Helper class. That way they can be used from the object and the
# otc db download. However check comments in otc db downloads for how these methods can
# be improved, and put those improved method(s) into the Helper class.
...
# LLD - make this into a helper(little bit of refactoring of args required)
...
# LLD is there no way to filter the actual API call so less has to be downloaded?
# should also be renamed and functionality clarified.  Possibly a Helper method
    def filter_timetable_dict(self, service_code):
        ...
        filtered_dict = {
            k: v for k, v in self.timetable_dict.items() if service_code in k
        }
...
# LLD we don't need to set the Excel limit. We can set a warning r.e. excel, but a csv
# is not an excel file and does not itself truncate the data. Python users/analysts are likely
# to be using csv as df, which also does not truncate the data
...
# LLD save destination can be an attribute of the initiator, this would solve lots of other
# structural problems and also enable a default to be set or user to pass own download
# location

# LLD you can also have one save function, with arguments - e.g.
# export_to_csv(metadata=true, timetables_individual=True, timetables_combined=True,
# save_to_downloads=True, filepath=True) etc. This would be more valuable than all these
# separate methods, as the user can actually just save the variable df to csv and set their
# own preferred location without all this extra code. We can still include a default location
# if deemed necessary
# your doc string can describe how to use this, as well as online example.
# This would 
    ...
    def save_service_line_extract_to_csv(self):
        ...
        destination = TimetableExtractor.create_today_folder(self)
        self.service_line_extract.to_csv(
            f"{destination}/service_line_extract.csv", index=False
        )
    
# LLD - combine, see above
...
# LLD not super useful - also needs docstring to better explain how it would  be used.
# how come to_csv can't be used for this one? use the default destination attribute
...
# LLD rename to DQ reporting. Consider what could be combined into one 
# 'DQ report' function, with a printed df output.
    # =============================================================================
    #       REPORTING FUNCTIONS
    # =============================================================================
...
# LLD use md = self.metadata.copy(). helps readability and not changing the stored df
        self.metadata["dq_score"] = self.metadata["dq_score"].astype(str)
        self.metadata["dq_score"] = (
            self.metadata["dq_score"].str.rstrip("%").astype("float")
        )
...
# LLD combine into DQ function
    def no_licence_no(self):
        ...
        grouped = self.service_line_extract_with_stop_level_json[
            self.service_line_extract_with_stop_level_json["LicenceNumber"].map(len)
            == 0
        ]
...
# LLD for both of this use .dropna().unique()))
...
# LLD add in doc string how it checks if a service code is valid - i.e. all the rules checked one by one.
# LLD because of write to memory efficiency, probably better to combine all the below into one well-formatted
# 'and' filter, instead of multiple filters. You don't need to check a Boolean function for True.
    def valid_service_codes(self):
        ...
        correct_service_code = self.service_line_extract_with_stop_level_json[
            self.service_line_extract_with_stop_level_json["ServiceCode"]
            .str[:2]
            .str.isalpha()
            # LLD line below not needed
            == True
        ]
        # 10th is colon
        correct_service_code = correct_service_code[
            correct_service_code["ServiceCode"].str[9:10] == ":"
        ]
        # > than 10 characters
        correct_service_code = correct_service_code[
            correct_service_code["ServiceCode"].str.len() > 10
        ]

        # LLD correct me if I'm being blind, but isn't code for reg and unreg exactly the same? 
        # is this the desired functionality?
        # unreg must be handled differently
        correct_service_code_unreg = correct_service_code[
            correct_service_code["ServiceCode"].str[:2] == "UZ"
        ]
        correct_service_code_reg = correct_service_code[
            correct_service_code["ServiceCode"].str[:2] != "UZ"
        ]
        # next 7 are int unless first two of whole thing are UZ
        correct_service_code_reg = correct_service_code_reg[
            correct_service_code_reg["ServiceCode"].str[2:9].str.isnumeric() #LLD not needed == True
        ]
        # right after colon are int, unless first two of whole thing are UZ
        correct_service_code_reg = correct_service_code_reg[
            correct_service_code_reg["ServiceCode"].str[10:].str.isnumeric() #LLD not needed == True
        ]

        # LLD why are they combined?
        correct_service_code_final = pd.concat(
            [correct_service_code_reg, correct_service_code_unreg]
        )
        ...
        invalid_serv = all_serv[
            ~all_serv.apply(tuple, 1).isin(valid_serv.apply(tuple, 1))
        ]
        invalid_service_codes = invalid_serv.merge(
            self.service_line_extract_with_stop_level_json, how="left", on="ServiceCode"
        )

        unique_valid_service_codes = len(
            correct_service_code_final["ServiceCode"].unique()
        )
        print(
...
# LLD explain in docstring what txc 2 4 actually is, and where user can go for more details
    def services_published_in_TXC_2_4(self):
        ...
        # LLD this method can be improved as:
        sl = self.service_line_extract_with_stop_level_json.copy() #btw this shouldn't really be called json still if it's now a df
        # copy in case sl ever gets changed in an amended version of this method

        schema_filter = sl["SchemaVersion"] == "2.4"
        TXC_2_4_schema, not_TXC_2_4_schema = sl[schema_filter], sl[~schema_filter]

        perc_compliant = (len(TXC_2_4_schema)/len(sl))*100

        # count_published_in_2_4_schema = len(
        #     self.service_line_extract_with_stop_level_json[
        ...
        #     self.service_line_extract_with_stop_level_json["SchemaVersion"] != "2.4"
        # ]
        # perc_published_in_2_4_schema = (
        #     count_published_in_2_4_schema
        #     / len(self.service_line_extract_with_stop_level_json)
        # ) * 100
        print(
            f"\nPercentage of services published in TXC 2.4 schema: {perc_compliant}\n"
        )
        return perc_compliant, TXC_2_4_schema, not_TXC_2_4_schema
        # LLD please can you amend similar methods, e.g. the one below, to use this format.
    ...
    def datasets_published_in_TXC_2_4(self):
        ...
        datasets_schema_pre["SchemaVersion"] = datasets_schema_pre[
            "SchemaVersion"
        ].astype("float")
        datasets_schema = datasets_schema_pre.groupby("DatasetID").agg(
            {"SchemaVersion": "min"}
        )
        count_datasets_published_in_2_4_schema = len(
            datasets_schema[datasets_schema["SchemaVersion"] == 2.4]
        )
        perc_datasets_published_in_2_4_schema = (
            count_datasets_published_in_2_4_schema / len(datasets_schema)
        ) * 100
        TXC_2_4_schema = self.service_line_extract_with_stop_level_json[
            self.service_line_extract_with_stop_level_json["SchemaVersion"] == "2.4"
        ]
...
# LLD only run OTC reporting if requested. One overall orchestrating method will do it.
# LLD REVIEW THIS SECTION MONDAY
    def licence_from_sc(self):
        ...
        registered_licence_from_sc = self.otc_db["service_code"].str.split(
            ":", expand=True
        )
    ...
    def registered_published_services_all(self):
        ...
        published_licence_from_sc = self.service_line_extract["ServiceCode"].str.split(
            ":", expand=True
        )
        ...
        all_services = pd.DataFrame(
            number_registered_services_published.items(),
    ...
    def registered_not_published_services(self):
        ...
        full_merge = self.otc_db[
            ["service_code", "op_name", "lic_no", "auth_description"]
        ].merge(
            service_line_lite,
            how="outer",
        ...
        not_published = not_published_full[
            ["service_code", "op_name", "lic_no", "auth_description"]
        ]
    ...
    def services_on_bods_or_otc_by_area(self):
        ...
        try:
            ...
            atco_lookup_file = importlib.resources.read_text(
            ...
            la_lookup = pd.read_csv(
                "ATCO_code_to_LA_lookup.csv", dtype={"ATCO Code": str}
            )
        ...
        otc_la_merge = (
            otc[["service_code", "service_number", "op_name", "auth_description"]]
            .merge(
                la_lookup[["Auth_Description", "ATCO Code"]],
                how="left",
                right_on="Auth_Description",
                left_on="auth_description",
            )
            .drop_duplicates()
        )
        ...
        bods_la_merge = (
            self.service_line_extract_with_stop_level_json[
                ["ServiceCode", "LineName", "la_code", "OperatorName"]
            ]
            .merge(
                la_lookup[["Admin Area Name associated with ATCO Code", "ATCO Code"]],
                how="left",
                right_on="ATCO Code",
                left_on="la_code",
            )
        ...
        full_service_code_with_atco = (
            otc_la_merge[["service_code", "LineName", "op_name", "ATCO Code", "in"]]
            .add_suffix("_otc")
            .merge(
                bods_la_merge.add_suffix("_bods"),
                how="outer",
                right_on=["ServiceCode_bods", "ATCO Code_bods"],
                left_on=["service_code_otc", "ATCO Code_otc"],
            )
            .drop_duplicates()
        )

        # coalesce service code and atco code cols
        full_service_code_with_atco["service_code"] = full_service_code_with_atco[
            "service_code_otc"
        ].combine_first(full_service_code_with_atco["ServiceCode_bods"])
        full_service_code_with_atco["atco_code"] = full_service_code_with_atco[
            "ATCO Code_otc"
        ].combine_first(full_service_code_with_atco["ATCO Code_bods"])
        ...
        full_service_code_with_atco["in_otc"] = full_service_code_with_atco[
            "in_otc"
        ].fillna(0)
        full_service_code_with_atco["in_bods"] = full_service_code_with_atco[
            "in_bods"
        ].fillna(0)
        ...
        full_service_code_with_atco["LineName_bods"] = full_service_code_with_atco[
            "LineName_bods"
        ].fillna("xxxxx")
        ...
        full_service_code_with_atco = full_service_code_with_atco.groupby(
            ...
            as_index=False,
            dropna=False,
        ).agg({"LineName_bods": lambda x: ",".join(x)})
    ...
    def services_on_bods_or_otc_by_area_mi(self):
        ...
        full_service_code_with_atco = (
            TimetableExtractor.services_on_bods_or_otc_by_area(self)
        )
        service_atco_mi = full_service_code_with_atco.groupby("atco_code").agg(
            {
                "service_code": "count",
    ...
    def services_on_bods_or_otc_by_area_just_otc(self):
        ...
        full_service_code_with_atco = full_service_code_with_atco[
            full_service_code_with_atco["in_otc"] == 1
        ]
    ...
    def services_on_bods_or_otc_by_area_mi_just_otc(self):
        ...
        full_service_code_with_atco = (
            TimetableExtractor.services_on_bods_or_otc_by_area_just_otc(self)
        )
        service_atco_mi = full_service_code_with_atco.groupby("atco_code").agg(
            {
                "service_code": "count",
...
# LLD REVIEWED ALL BELOW THIS LINE ---------------
...
# LLD these should all be combined into one method, with the argument being e.g. NationalOperatorCode,
# TradingName etc. You may wish to provide a method to allow user to check what the arguments can be.
# You will need to explain namespace to user and location of constants.py (presumably this has been
# replaced by self.Namespace?) If so, think of reuse for when fares data is available (essentially,
# method will need to take string and namespace arguments)
    ...
    def extract_line_name(self):
        ...
        # find all text in the given xpath, return as a element object
        data = self.root.findall(
            f"Services//Service/Lines/Line/LineName", self.namespace
        )
    ...
    def extract_service_origin(self):
        ...
        # find all text in the given xpath, return as a element object
        data = self.root.findall(
            f"Services//Service/StandardService/Origin", self.namespace
        )
    ...
    def extract_service_destination(self):
        ...
        # find all text in the given xpath, return as a element object
        data = self.root.findall(
            f"Services//Service/StandardService/Destination", self.namespace
        )
    ...
    def extract_operating_period_start_date(self):
        ...
        # find all text in the given xpath, return as a element object
        data = self.root.findall(
            f"Services//Service/OperatingPeriod/StartDate", self.namespace
        )
...
# LLD should this remain undefined?
    def extract_operating_period_end_date(self):
        ...
        # find all text in the given xpath, return as a element object
        data = self.root.findall(
            f"Services//Service/OperatingPeriod/EndDate", self.namespace
        )

        operating_period_end_date = [
            i.text if len(i.text) > 0 else "No Data" for i in data
        ]
...
# LLD similarly, combine all of the below code into one function (will have an if block for
# running la code)
